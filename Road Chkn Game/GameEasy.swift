...
struct GameEasy: View {
    ...
    @State private var requiredClicks: Int = 30 // Необходимое количество кликов для победы
    ...
    private func startGame() {
        ...
        gameTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
            if !isPaused { // Продолжаем только если не на паузе
                timer -= 1
                carPosition += 1.0 / 30.0 // Машина доходит до конца за 30 секунд
                checkGameResult()
                if timer <= 0 {
            ...
            VStack(spacing: -30) {
                Button(action: {}) {
                    Image("retryBtn")
                        ...
                        .onTapGesture {
                            coinscore += 10
                            NavResult.shared.currentScreen = .LEVELS
                        }
                }
...
struct PauseView: View {
    let resumeAction: () -> Void // Функция для возобновления игры
            ...
            VStack(spacing: -40) {
                Button(action: {}) {
                    Image("resumeBtn")
                        ...
                        .onTapGesture {
                            resumeAction() // Возвращаемся к игре
                        }
                }
